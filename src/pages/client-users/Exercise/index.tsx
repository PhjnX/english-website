...
import Paragraph from "../ReadingTestPage/Paragraph";
import Question from "../ReadingTestPage/Question";
import { motion, AnimatePresence } from "framer-motion";
import { getExerciseData } from "../../../data/ExerciseListData";
export interface Answers {
  [key: number]: string;
}

const ExercisePage: React.FC = () => {
  const { level, exercise } = useParams();
  ...
  // Lấy dữ liệu động theo level/exercise
  const { parts } = getExerciseData(level, exercise);
  ...
  const handleSubmit = () => {
    ...
    navigate("/reading-score", {
      state: {
        ...
        isSubmitted: true,
        questions: parts.flatMap((p: any) => p.questions),
      },
    });
  };
  ...
  if (!parts)
    return (
      <div className="p-8 text-red-500">Không tìm thấy dữ liệu bài test.</div>
    );
